...
export default function InterviewRoom() {
  ...
  useEffect(() => {
    api.get(`/candidates/session/${sessionId}`).then(r => {
      ...
      const intv = { ...r.data.session.interview };
      intv.questions = r.data.session.questions || [];
      setInterview(intv);
  ...
  const submitAnswer = async (skip = false) => {
    ...
    try {
      ...
      if (currentQIndex + 1 >= (interview?.numberOfQuestions || 10)) {
        await completeInterview();
      } else {
        if (res.data.nextQuestion) {
          setInterview(prev => ({ ...prev, questions: [...(prev.questions || []), res.data.nextQuestion] }));
        }
    ...
    <div style={{ minHeight: '100vh', background: 'var(--bg-primary)', padding: 'var(--space-lg)' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24, padding: '0 8px' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          ...
          <div>
            ...
            <div style={{ fontSize: '0.8rem', color: 'var(--text-muted)' }}>Question {currentQIndex + 1} of {interview?.numberOfQuestions || 10}</div>
          </div>
        </div>
      ...
      <div style={{ marginBottom: 24 }}>
        <div className="progress-bar-wrapper">
          <div className="progress-bar-fill" style={{ width: `${((currentQIndex + 1) / (interview?.numberOfQuestions || 10)) * 100}%` }} />
        </div>
        <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 6, fontSize: '0.75rem', color: 'var(--text-muted)' }}>
          <span>Progress: {currentQIndex + 1}/{interview?.numberOfQuestions || 10} questions</span>
      ...
      <div className="interview-room">
        ...
        <div className="question-panel">
          ...
          <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
            ...
            {Array.from({ length: Math.max(0, (interview?.numberOfQuestions || 10) - questions.length) }).map((_, i) => (
              <div key={questions.length + i} style={{
                width: 10, height: 10, borderRadius: '50%',
        ...
        <div className="answer-panel">
          ...
          <div style={{ display: 'flex', gap: 12 }}>
            ...
            >
              ...
              ) : currentQIndex + 1 >= (interview?.numberOfQuestions || 10) ? (
                <>Finish Interview ✓</>
              ) : (
